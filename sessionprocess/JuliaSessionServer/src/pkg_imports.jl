# Vendored dependencies, loaded by direct `include` so that nothing has to be resolved from
# a registry and nothing leaks into the user's own environment. The version tiers mirror
# the packages that still support each Julia version; see `packages-old/`.
